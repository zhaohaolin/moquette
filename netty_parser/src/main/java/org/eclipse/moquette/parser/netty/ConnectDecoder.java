...
public class ConnectDecoder extends DemuxDecoder {
	...
	static final AttributeKey<Boolean>	CONNECT_STATUS	= AttributeKey
																.valueOf("connected");
	
	@Override
	void decode(AttributeMap ctx, ByteBuf in, List<Object> out)
		...
		switch (protocolNameLen) {
			case 6:
				...
				if (!"MQIsdp".equals(protoName)) {
					in.resetReaderIndex();
					throw new CorruptedFrameException("Invalid protoName: "
							+ protoName);
				}
			...
			default:
				// protocol broken
				throw new CorruptedFrameException("Invalid protoName size: "
						+ protocolNameLen);
		}
		...
		if (message.getProtocolVersion() == VERSION_3_1_1) {
			...
			if (message.isDupFlag() || message.isRetainFlag()
				...
				throw new CorruptedFrameException(
						"Received a CONNECT with fixed header flags != 0");
			}
			
			// check if this is another connect from the same client on the same
			// session
			Attribute<Boolean> connectAttr = ctx
					.attr(ConnectDecoder.CONNECT_STATUS);
			...
			if (alreadyConnected == null) {
				...
				throw new CorruptedFrameException(
						"Received a second CONNECT on the same network connection");
			}
		}
		...
		if (message.getProtocolVersion() == VERSION_3_1_1) {
			if ((connFlags & 0x01) != 0) { // bit(0) of connection flags is != 0
				throw new CorruptedFrameException(
						"Received a CONNECT with connectionFlags[0(bit)] != 0");
			}
		}
