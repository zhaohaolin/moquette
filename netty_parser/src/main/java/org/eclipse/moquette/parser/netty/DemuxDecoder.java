...
abstract class DemuxDecoder {
	...
	abstract void decode(AttributeMap ctx, ByteBuf in, List<Object> out)
			throws Exception;
	
	/**
	 * Decodes the first 2 bytes of the MQTT packet. The first byte contain the
	 * packet operation code and the flags, the second byte and more contains
	...
	/**
	 * Do the same as the @see#decodeCommonHeader but having a strong validation
	 * on the flags values
	 */
	protected boolean decodeCommonHeader(AbstractMessage message,
			int expectedFlags, ByteBuf in) {
		return genericDecodeCommonHeader(message, expectedFlags, in);
	}
	
	private boolean genericDecodeCommonHeader(AbstractMessage message,
			Integer expectedFlagsOpt, ByteBuf in) {
		...
		try {
			...
			throw new CorruptedFrameException(String.format(
					"Received an invalid QOS: %s", e.getMessage()), e);
		}
