	...
	void decode(AttributeMap ctx, ByteBuf in, List<Object> out)
		...
		if (message.getQos() != AbstractMessage.QOSType.LEAST_ONE) {
			throw new CorruptedFrameException(
					"Found an Unsubscribe message with qos other than LEAST_ONE, was: "
							+ message.getQos());
		}
		...
		while (read < message.getRemainingLength()) {
			...
			if (topicFilter.length() == 0) {
				throw new CorruptedFrameException(
						"Received an UNSUBSCRIBE with empty topic filter");
			}
		...
		if (message.topicFilters().isEmpty()) {
			throw new CorruptedFrameException(
					"unsubscribe MUST have got at least 1 topic");
		}
		out.add(message);
	}
