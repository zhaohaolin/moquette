...
class SubscribeDecoder extends DemuxDecoder {
	...
	void decode(AttributeMap ctx, ByteBuf in, List<Object> out)
		...
		if (message.subscriptions().isEmpty()) {
			throw new CorruptedFrameException(
					"subscribe MUST have got at least 1 couple topic/QoS");
		}
	...
	private void decodeSubscription(ByteBuf in, SubscribeMessage message)
		...
		if (topic.length() == 0) {
			throw new CorruptedFrameException(
					"Received a SUBSCRIBE with empty topic filter");
		}
		byte qosByte = in.readByte();
		if ((qosByte & 0xFC) > 0) { // the first 6 bits is reserved => has to be
									// 0
			throw new CorruptedFrameException(
					"subscribe MUST have QoS byte with reserved buts to 0, found "
