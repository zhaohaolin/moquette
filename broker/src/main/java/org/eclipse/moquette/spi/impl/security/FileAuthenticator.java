 ...
 * </pre>
 * 
 * NB -n is important because echo append a newline by default at the of string.
 * -n avoid this behaviour.
 * 
 * @author andrea
 */
public class FileAuthenticator implements IAuthenticator {
	...
	public FileAuthenticator(String parent, String filePath) {
		...
		if (file.isDirectory()) {
			LOG.warn(String
					.format("Bad file reference %s is a directory", file));
			return;
		}
		try {
		...
		try {
			...
			LOG.warn(String.format("Format error in parsing password file %s",
					file), pex);
		}
	}
	...
	public boolean checkValid(String username, byte[] password) {
		...
		String encodedPasswd = new String(Hex.encodeHex(digest1));
