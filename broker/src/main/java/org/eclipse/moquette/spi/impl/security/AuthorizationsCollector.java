...
class AuthorizationsCollector implements IAuthorizator {
	...
	private static final Logger					LOG								= LoggerFactory
																						.getLogger(AuthorizationsCollector.class);
		...
		switch (keyword) {
			...
			default:
				throw new ParseException(String.format(
						"invalid line definition found %s", line), 1);
		}
	}
	
	private Authorization createAuthorization(String line, String[] tokens)
			throws ParseException {
		if (tokens.length > 2) {
			// if the tokenized lines has 3 token the second must be the
			// permission
			try {
	...
	private boolean canDoOperation(String topic,
			Authorization.Permission permission, String username, String client) {
	...
	private boolean matchACL(List<Authorization> auths, String topic,
