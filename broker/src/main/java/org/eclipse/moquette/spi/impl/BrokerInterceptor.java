...
final class BrokerInterceptor implements Interceptor {
	...
	public void notifyClientDisconnected(final String clientID) {
		for (final InterceptHandler handler : this.handlers) {
			executor.execute(new Runnable() {
				@Override
				public void run() {
					handler.onDisconnect(new InterceptDisconnectMessage(
							clientID));
				}
			});
		}
	...
	@Override
	public void notifyTopicUnsubscribed(final String topic,
			final String clientID) {
		for (final InterceptHandler handler : this.handlers) {
			executor.execute(new Runnable() {
				@Override
				public void run() {
					handler.onUnsubscribe(new InterceptUnsubscribeMessage(
							topic, clientID));
				}
			});
		}
